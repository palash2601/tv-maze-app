...
describe("ShowsList", () => {
  ...
  describe("carousel visibility is true", () => {
    const props = {
      isLoading: false,
      shows: [1, 2, 3, 4, 5, 6].map((item) => ({ ...showMockData, id: item })),
    };
    const mountConfigs = {
      global: {
        stubs: {
  ...
  describe("carousel visibility is false", () => {
    const props = {
      isLoading: false,
      shows: [1, 2].map((item) => ({ ...showMockData, id: item })),
    };
    const mountConfigs = {
      global: {
        stubs: {
  ...
  describe("carousel visibility", () => {
    ...
    it("should be false when width > 1024 & item < 4", async () => {
      const wrapper = mount(ShowsList, {
        ...
        props: {
          isLoading: false,
          shows: [1, 2, 3].map((item) => ({ ...showMockData, id: item })),
        },
      });
      await wrapper.vm.$nextTick();
      expect(wrapper.find('[data-test="Carousel"]').exists()).toBe(false);
    ...
    it("should be false when width > 1024 & item = 4", async () => {
      const wrapper = mount(ShowsList, {
        ...
        props: {
          isLoading: false,
          shows: [1, 2, 3, 4].map((item) => ({ ...showMockData, id: item })),
        },
      });
      await wrapper.vm.$nextTick();
      expect(wrapper.find('[data-test="Carousel"]').exists()).toBe(false);
